...
public class LibraryLoader {
    ...
    public ClassLoader createLoader(@NotNull PluginDescriptionFile desc) {
        IPluginDescriptionFile duck = (IPluginDescriptionFile) (Object) desc;
        if (duck.getLibraries().isEmpty()) {
            return null;
        }
        logger.log(Level.INFO, "[{0}] Loading {1} libraries... please wait", new Object[]{desc.getName(), duck.getLibraries().size()});

        List<Dependency> dependencies = new ArrayList<>();
        for (String library : duck.getLibraries()) {
