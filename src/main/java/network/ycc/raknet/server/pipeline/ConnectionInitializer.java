        ...
        switch (state) {
            ...
            case CR2: {
                if (msg instanceof ConnectionRequest) {
                    ...
                    ctx.channel().attr(RakNet.HYBRIDFIX_PROTOCOL_EXTENSION)
                            .set(cr.hasHybridFixProtocolExtension());
                    final Packet packet = new ServerHandshake(
                            (InetSocketAddress) ctx.channel().remoteAddress(),
                            cr.getTimestamp(),
                            20,
                            cr.hasHybridFixProtocolExtension());
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) {
            if (msg instanceof Packet.ClientIdConnection) {
                ...
                if (clientIdConnectionPacket.getClientId() == config.getClientId()) {
                    HybridFix.LOGGER.warn("Warning: Not restarting RakNet connection for client {} (Possibly caused by high latency)", ctx.channel().remoteAddress());
                } else {
